...
def _clean_hashtags_and_urls(df):
    ...
    df["Url"] = df.apply(lambda row: 'https://twitter.com/'+ str(row["screen_name"]) +'/status/'+ str(row['id']), axis=1)
...
def _row_to_doc_dict(row: pd.Series):
    _corpus[row['Id']] = Document(row['Id'], row['Tweet'][0:100], row['Username'], row['Tweet'], row['Date'], row['Likes'],
                                  row['Retweets'],
